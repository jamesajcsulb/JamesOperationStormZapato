...
public class BaseStripeFunctionalTestMain {
   ...
   // @Before
    ...
    protected void testMetadata(MetadataStore<?> object) throws StripeException {
        Assert.assertTrue(object.getMetadata().isEmpty());
        ...
        Assert.assertFalse(object.getMetadata().isEmpty());
        ...
        object = object.update(updateParams);
        Assert.assertFalse(object.getMetadata().isEmpty());
        Assert.assertFalse(object.getMetadata().containsKey("foo"));
        assertEquals("qux", object.getMetadata().get("baz"));
